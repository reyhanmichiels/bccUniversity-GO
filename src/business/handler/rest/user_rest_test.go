...
func TestDropClassPath1(t *testing.T) {

	for i := 1; i <= 5; i++ {

		t.Run(fmt.Sprintf("path 1 drop class testing %d", i), func(t *testing.T) {
			...
			assert.Equal(t, "error", jsonResponse["status"], "status should be equal")
			assert.Equal(t, "failed convert id to int", jsonResponse["message"], "message should be equal")
